...
export const metadata: Metadata = {
  title: "Community",
  description:
    "Cộng đồng EurusDevSec — nơi chia sẻ kiến thức DevSecOps, Cloud và cuộc sống.",
};
...
export default async function CommunityPage() {
  ...
  const { data: posts } = await supabase
    .from("posts")
    .select(
      "id, title, slug, excerpt, tags, created_at, author_id, cover_image",
    )
    .eq("status", "published")
  ...
  if (posts && posts.length > 0) {
    ...
    profiles?.forEach((p) => authorMap.set(p.id, p));
  }
